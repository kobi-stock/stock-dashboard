...
const WS_BASE =
  ...
  `${window.location.protocol === "https:" ? "wss" : "ws"}://${API_HOST}:8000`;
...
function normalizeRealtimeTicker(ticker) {
  const text = normalizeTicker(ticker);
  if (/^\d{6}$/.test(text)) return text;
  const match = text.match(/^(\d{6})\.(KS|KQ)$/);
  if (match) return match[1];
  return null;
}

function getMarketStatusTone(status) {
  switch (status?.code) {
    case "regular":
      return { color: "#22c55e", bg: "rgba(34,197,94,0.14)", border: "rgba(34,197,94,0.32)" };
    case "after_close":
    case "single_price":
    case "pre_close":
      return { color: "#f59e0b", bg: "rgba(245,158,11,0.14)", border: "rgba(245,158,11,0.32)" };
    default:
      return { color: "#94a3b8", bg: "rgba(148,163,184,0.12)", border: "rgba(148,163,184,0.24)" };
  }
}

function getMarketStatusNote(status) {
  if (!status) return "상태 확인 중";
  if (status.priceMode === "websocket") return "국내 종목은 웹소켓으로 빠르게 반영";
  if (status.code === "after_close" || status.code === "single_price" || status.code === "pre_close") {
    return "국내 종목은 5초 주기 조회로 시간외 가격 반영";
  }
  if (status.code === "after_close_wait") return "주문접수 구간, 체결 가격 변동은 잠시 대기";
  return "장중이 아니어서 마지막 체결가 중심으로 표시";
}

function mergeQuoteIntoItem(item, quote) {
  if (!item || !quote) return item;
  const itemCode = normalizeRealtimeTicker(item.ticker);
  const quoteCode = normalizeRealtimeTicker(quote.ticker);
  const sameTicker =
    normalizeTicker(item.ticker) === normalizeTicker(quote.ticker) ||
    (itemCode && quoteCode && itemCode === quoteCode);

  if (!sameTicker) return item;
  return {
    ...item,
...
function applyRealtimeQuote(items, quote) {
  if (!Array.isArray(items) || !quote) return items;
  return items.map((item) => mergeQuoteIntoItem(item, quote));
}
...
export default function App() {
  ...
  async function fetchMarketStatus() {
    try {
      const data = await fetchJson("/market-status");
      setMarketStatus(data?.krx || null);
      return true;
    } catch (error) {
      console.error("fetchMarketStatus", error);
      return false;
    }
  }
  ...
  async function refreshAll() {
    ...
    const okStatus = await fetchMarketStatus();
    ...
    const successes = [okMarket, okStatus, okNews, okDefault, okExtra, okHoldings].filter(Boolean).length;
    if (successes === 0) {
  ...
  useEffect(() => {
    ...
    function connectRealtimeSocket() {
      ...
      socket.onopen = () => {
        const tickers = [
          ...stockItemsRef.current.map((item) => item.ticker),
          ...extraStocksRef.current.map((item) => item.ticker),
          ...holdingsRef.current.map((item) => item.ticker),
        ]
          .map((ticker) => normalizeRealtimeTicker(ticker))
          .filter(Boolean);
      ...
      socket.onmessage = (event) => {
        try {
          ...
          for (const quote of items) {
            setStockItems((prev) => {
              const next = applyRealtimeQuote(prev, quote);
            ...
            setSelectedStock((prev) => (prev ? mergeQuoteIntoItem(prev, quote) : prev));
            setSelectedInvestStock((prev) => (prev ? mergeQuoteIntoItem(prev, quote) : prev));
          }
        } catch (error) {
          console.error("realtime onmessage", error);
        }
      };
      ...
      socket.onclose = () => {
        ...
        if (!closedByCleanup) {
          reconnectTimer = window.setTimeout(connectRealtimeSocket, 2000);
        }
      };
  ...
  useEffect(() => {
    ...
    const tickers = [
      ...stockItems.map((item) => item.ticker),
      ...extraStocks.map((item) => item.ticker),
    ...
    <div className="app-shell">
      <div className="app-container">
        <header className="app-header">
          <h1>증권 대시보드</h1>
          <p className="header-desc">국내 종목은 장중에는 웹소켓, 시간외에는 5초 주기 조회로 반영합니다.</p>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "stretch" }}>
            ...
            <div
              ...
              style={{
                minWidth: 220,
                border: `1px solid ${getMarketStatusTone(marketStatus).border}`,
                background: getMarketStatusTone(marketStatus).bg,
              }}
            >
              <div>KRX 장상태</div>
              <strong style={{ color: getMarketStatusTone(marketStatus).color }}>
                {marketStatus?.label || "확인 중"}
              </strong>
          ...
          <div style={{ marginTop: 8, fontSize: 12, color: "#cbd5e1" }}>
            {getMarketStatusNote(marketStatus)}
          </div>
